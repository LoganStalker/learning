...
FFmenu = pygame.font.Font('DejaVuSans-ExtraLight.ttf', n*8) # большой шрифт
...
def game(matrix):
    ...
    while done:
        ...
        else: # если номер победителя не равен нулю (winner[0] != 0)
...
while done:
    ...
    for e in pygame.event.get():
        if e.type == pygame.KEYDOWN:
            if e.key == pygame.K_ESCAPE: # если нажата клавиша ESC, то программа завершится (окно закроется)
                done = False
            if e.key == pygame.K_RETURN: # если нажата клавиша ENTER, то будет запущен игровой цикл
    ...
    screen.blit(FFmenu.render(u"Enter - игра", 1, (255, 255, 0, 50)), (screen.get_width()/3.5, screen.get_width()/2.8))
    screen.blit(FFmenu.render(u"Esc   - выход", 1, (255, 255, 0, 50)), (screen.get_width()/3.8, screen.get_width()/2))
